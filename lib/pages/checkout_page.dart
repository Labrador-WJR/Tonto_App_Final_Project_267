...
class _CheckoutPageState extends State<CheckoutPage> {
  ...
  void initState() {
    ...
    // --- ADDED: Fetch the user's default address from Supabase when page loads ---
    _fetchDefaultAddress();
  }

  // --- NEW: Function to grab the default address ---
  Future<void> _fetchDefaultAddress() async {
    try {
      ...
      final res = await _supabase
          ...
          .eq('user_id', user.id)
          .eq('is_default', true)
          .maybeSingle(); // Gets the default address if it exists

      if (res != null && mounted) {
        setState(() {
          selectedAddress = res['address_string'];
        });
      }
    } catch (e) {
  ...
  Future<void> _placeOrder(double grandTotal) async {
    // --- NEW: Hard block to prevent checkout without an address ---
    if (selectedAddress == 'No address selected' || selectedAddress.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('⚠️ Please select a shipping address before checking out!'),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          // --- UPDATED: Turns RED if no address is selected ---
                          Text(
                            selectedAddress, 
                            style: TextStyle(
                              color: selectedAddress == 'No address selected' ? Colors.redAccent : Colors.white, 
                              fontSize: 16, 
                      ...
                      child: Row(
                        children: [
                          ...
                          Container(
                            ...
                            child: item.imagePath.isNotEmpty && item.imagePath != 'placeholder'
                                ? ClipRRect(
                                    borderRadius: BorderRadius.circular(6),
                                    child: Image.network(item.imagePath, fit: BoxFit.cover, errorBuilder: (c, e, s) => const Icon(Icons.image_not_supported, color: Colors.grey)),
                                  )
